...
> **Skill inversa:** `skills:html-to-figma` faz o caminho contrário (código/HTML → Figma).
...
- `CLAUDE.md` lido antes de qualquer implementação
...
Nenhum código gerado pode contradizer `CLAUDE.md`.
...
Retorna:
- Propriedades de Auto Layout (layout, constraints, sizing)
...
- [ ] Identificar o framework do projeto (React, Vue, HTML puro, etc.) via `CLAUDE.md`
...
#### Usar a skill `skills:frontend-design` em conjunto

Para garantir qualidade estética e acessibilidade no código gerado, aplicar os princípios da skill `skills:frontend-design`:
...
- Colocar componentes UI no diretório definido em `CLAUDE.md`
...
5. Verify implementation complete before handing off to test-runner
...
```
1. Ler CLAUDE.md → projeto usa React + CSS Modules
...
```
1. Ler CLAUDE.md → projeto usa Next.js + styled-components
...
**Solução:** Tratar o output como referência de estrutura e valores — traduzir para o framework real do projeto conforme `CLAUDE.md`.
...
- [x] Convenções do CLAUDE.md seguidas
```
