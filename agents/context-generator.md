...
| `context/TESTING-POLICY.md` | `templates/testing-policy-md.md` | Risk-based what-to-test / what-not-to-test policy + triage gate | `/test-generator` (always), `/test-runner` |
...
/context-generator --testing    # context/TESTING-POLICY.md only
...
| context/TESTING-POLICY.md | ❌ Missing | |
```
...
|------------|----------------|
| Full-stack web | All 10 files |
| Backend API only | CLAUDE.md, context/ARCH.md, context/FOLDER_ARCH.md, context/API.md, context/DATA_MODEL.md, context/DECISIONS.md, context/GOTCHAS.md, context/ENVIRONMENT.md, context/TESTING-POLICY.md |
| Frontend only | CLAUDE.md, context/FOLDER_ARCH.md, context/DESIGN.md, context/API.md (if external APIs), context/DECISIONS.md, context/GOTCHAS.md, context/TESTING-POLICY.md |
| CLI / library | CLAUDE.md, context/FOLDER_ARCH.md, context/DECISIONS.md, context/GOTCHAS.md, context/ENVIRONMENT.md, context/TESTING-POLICY.md |
| Mobile app | All 10 files |

context/TESTING-POLICY.md is relevant for **any** stack that has (or should have) tests — skip only for throwaway/PoC repos with no test suite.
...
Generate in order: CLAUDE.md → context/ARCH.md → context/FOLDER_ARCH.md → context/API.md → context/DATA_MODEL.md → context/DESIGN.md → context/DECISIONS.md → context/GOTCHAS.md → context/ENVIRONMENT.md → context/TESTING-POLICY.md
...
**Prerequisites:** Read CLAUDE.md §6 (Testing Strategy). Delegate a scan of the
test setup to `cavecrew-investigator` (test config files, existing test dirs,
CI test steps) to learn the real runner and where tests live.

The **universal framework is fixed** — do NOT rewrite the MUST-test / DO-NOT-test
categories, the triage gate, or the coverage stance. Only fill the project-specific
`[placeholders]`:

- `[PROJECT_CRITICAL_JOURNEYS]` — the real flows where breakage = money/data lost.
- `[PROJECT_BIZ_LOGIC]` — the actual branchy/security-sensitive symbols in this repo.
- `[FRAMEWORK]` / `[UNIT_TEST_RUNNER]` — the real UI framework and test runner.
- `[SECURITY_FN]` / `[BIZ_LOGIC_SYMBOL]` / `[STATIC_COMPONENT]` / `[INTERNAL_FLAG]` — real example rows for the triage table.
- `[COVERAGE_THRESHOLD]` — this project's number if it has one (else keep "no global gate").

**Questions:**
1. "Test runner(s)? (vitest / jest / pytest / go test / …)"
2. "2-4 critical user journeys where a break costs money/data or makes the product unusable?"
...
| context/TESTING-POLICY.md | ✅ Created | 90 |
